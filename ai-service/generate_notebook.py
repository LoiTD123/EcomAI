"""
Chạy script này để sinh file train_lstm.ipynb (v2):
...
# ── Cell 1: Install ───────────────────────────────────────────────────────
cells.append(md(["## 📦 Cell 1 — Cài thư viện\n"]))
cells.append(code([
...
cells.append(md(["## ⚙️ Cell 2 — Import & Cấu hình\n"]))
cells.append(code([
...
cells.append(md(["## 🏗️ Cell 3 — Kiến trúc Mô hình LSTM\n"]))
cells.append(code([
...
# ── Cell 4: Load Real Data ────────────────────────────────────────────────
cells.append(md(["## 📂 Cell 4 — Load Dữ liệu Hành vi Thực (SQLite)\n"]))
cells.append(code([
...
# ── Cell 5: Visualize Real Data ───────────────────────────────────────────
cells.append(md(["## 📊 Cell 5 — Phân tích Dữ liệu Thực\n"]))
cells.append(code([
    "if raw_rows:\n",
...
cells.append(md(["## 🔗 Cell 7 — Xây dựng Sequences Huấn luyện\n"]))
cells.append(code([
...
# ── Cell 8: Visualize Data Distribution ──────────────────────────────────
cells.append(md(["## 📊 Cell 8 — Visualize Phân phối Dữ liệu Huấn luyện\n"]))
cells.append(code([
...
# ── Cell 9: DataLoader ────────────────────────────────────────────────────
cells.append(md(["## 🔄 Cell 9 — Dataset & DataLoader (Train/Val Split 85/15)\n"]))
cells.append(code([
...
cells.append(md(["## 📉 Cell 11 — Visualize Quá trình Huấn luyện\n"]))
cells.append(code([
...
# ── Cell 12: Embedding Heatmap ────────────────────────────────────────────
cells.append(md(["## 🔥 Cell 12 — Heatmap Cosine Similarity Embedding\n",
...
cells.append(md(["## 🎯 Cell 13 — Demo Suy luận (Inference)\n"]))
cells.append(code([
...
cells.append(md([
    ...
    "| Nguồn | Mô tả |\n",
    "|---|---|\n",
    "| SQLite thực | Hành vi VIEW/ADD_TO_CART/PURCHASE từ user thật |\n",
    "| Persona tổng hợp | 4 phân khúc dựa theo catalog 21 sản phẩm thực |\n",
    "\n",
    "### Kỹ thuật Training\n",
    "| Kỹ thuật | Chi tiết |\n",
    "|---|---|\n",
    ...
    "| Checkpoint | Lưu khi val loss tốt nhất |\n",
    "\n",
    "### Tái huấn luyện\n",
    "```bash\n",
    "# Seed thêm data (giữ nguyên user thật, xóa user giả cũ)\n",
    "docker compose exec ai-service python seed_behaviors.py --users 1000 --clear\n",
    "\n",
    "# Train (tiếp tục từ checkpoint)\n",
    "docker compose exec ai-service python train_lstm.py\n",
    "```\n",
    "> 💡 Càng nhiều user thật tương tác → mô hình càng cá nhân hóa chính xác.\n"
]))
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "train_lstm.ipynb")
with open(out_path, "w", encoding="utf-8") as f:
    json.dump(notebook, f, ensure_ascii=False, indent=1)
print(f"[OK] Da tao notebook v2: {out_path}")
